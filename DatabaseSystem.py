    ...
    def get_vsm(self, tf, wqt):
        ...
        try:
            ...
            for index, document in enumerate(wqt):
                ...
                ww[index]['total'] = round(sqrt(total), 3)

            # ww_query merupakan key dan value query dalam ww
            ww_query = {}

            # a = indeks ke-0 dari ww atau dictionary milik query
            a = ww[0]
            # Untuk setiap key dan value dalam a (dict) {'kata1' : wqt^2, 'kata2' : wqt^2}
            for key, value in a.items():
                # Jika key bukan 'total' dan memiliki value > 0,
                # maka key-value dari ww_query akan ditambahkan dengan key dan value dari a
                if key != 'total' and value > 0:
                    ww_query[key] = value

            # Cara lain dengan menggunakan parameter ww dan clean_query
            #for word in clean_query.split(" "):
                #ww_query[word] = ww[0][word]

            # total_ww_doc merupakan key dan value dari 'total' dokumen dalam ww
            total_ww_doc = {}
            # total_ww_query = {'Bobot pencarian': value
            #                   'Bobot dokumen 1': value
            #                   'Bobot dokumen 2': value}
                ...
                if index > 0:
                    ...
                    for key, value in document.items():
                        ...
                        if key in ww_query:
                            ...
                            tf_ww[index-1][key] = value * ww_query[key]
            ...
            for index, document in enumerate(tf_ww):
                ...
                sum_tf_ww[f'Bobot dokumen {index+1}'] = total
            ...
            for x in range(length_list_content):
                ...
                cos = round(sum_tf_ww['Bobot dokumen ' + str(x+1)] / 
                            (total_ww_doc['Bobot pencarian'] * 
                            total_ww_doc['Bobot dokumen ' + str(x+1)]) , 3)
